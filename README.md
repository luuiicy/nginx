...
| 路径 | 说明 |
|---|---|
| `/etc/nginx/nginx.conf` | 主配置文件，负责全局配置和加载子配置 |
| `/etc/nginx/conf.d/*.conf` | 通用子配置目录，默认站点配置在 `default.conf` |
| `/etc/nginx/sites-available/` | 可用站点配置目录 |
| `/etc/nginx/sites-enabled/` | 生效站点配置目录，`nginx.conf` 会加载该目录下的文件 |
| `/etc/nginx/certs/` | SSL/TLS 证书目录 |
| `/etc/nginx/html/` | 默认静态首页目录 |

默认首页通过 SSI 输出服务器 IP。新增站点时，可将配置放到 `/etc/nginx/conf.d/*.conf`，或放到 `sites-available` 后再链接到 `sites-enabled`。
...
如需推送到 Oracle Cloud Container Registry，迪拜区域可使用 `dxb.ocir.io`，还需添加：
...
```text
dxb.ocir.io/<OCIR_NAMESPACE>/<OCIR_REPOSITORY>:latest
```

多架构镜像会生成一个 manifest list 和对应架构的子 manifest。为减少 OCIR 控制台中的 `unknown@sha256:...` 附属条目，Actions 已关闭 build provenance / SBOM attestation。
